...
const UpdateFlight = ({id}) => {
    ...
    useEffect(()=>{
        ...
        console.log(body);
        axios.put(`http://localhost:3000/api/v1/flights/${id}`,body).then(res=> console.log(res)).catch(err => console.log(err));
        setU(false)
    }
    ...
    <input type="text" id="NumberOfBusinessSeats" name="NumberOfBusinessSeats"  onChange={(event) => setNumberOfBusinessSeats(event.target.value)}/>
    <p><br></br></p>
    <label for="NumberOfFirstSeats">NumberOfFirstSeats:</label>
    <input type="text" id="NumberOfFirstSeats" name="NumberOfFirstSeats"  onChange={(event) => setNumberOfFirstSeats(event.target.value)}/>
    <p><br></br></p>
    ...
    <p><br></br></p> 
    <button type="Update Flight" value="Update Flight" onClick={sub} >Update flight </button>
  </form> )
